...
function buildSubsystems(
  ...
  const rows: Array<{ id: string; name: string; description: string; group: SettingGroup; tab: SubsystemTab; parent: string | null; settings: string[]; result: CardResult }> = [
    ...
    { id: 'eligibility', name: 'Activation eligibility rules', group: 'eligibility', tab: 'integration', parent: 'ringotel',
      description: 'Which extensions are excluded from Ringotel activation, by name, extension pattern, device heuristic, or directory visibility.',
      settings: ['RINGOTEL_API_KEY', 'RINGOTEL_ACTIVATION_SUFFIX', 'RINGOTEL_EXCLUDE_NAMES', 'RINGOTEL_EXCLUDE_EXTS', 'RINGOTEL_EXCLUDE_EXTS_BY_DOMAIN', 'RINGOTEL_EXCLUDE_NO_DEVICES', 'RINGOTEL_UNLISTED_USERS', 'RINGOTEL_RESELLER_OVERRIDE', 'RINGOTEL_PREPOP_INCLUDE_SOFT'],
      result: ringotelGateCard(env, errs.ringErr) },
