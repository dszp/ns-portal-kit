...
export const SUBSYSTEM_DETAIL: Record<string, string[]> = {
  ...
  identity: [
    ...
    '### It is the only stored credential here',
    'Everything else this Worker does runs as the calling user, on a token they supplied. This one exists because a change event arrives with nobody attached, so there is no caller to act as. That makes it the one credential worth restricting hard: NetSapiens can bound a key by model, domain and IP, and none of the caller-scope limits that bound every other write apply to it.',
    'The practical consequence: subscriptions do not work without a credential set on this card, and the change-event card above will stay inert with nothing on it that looks wrong.',
...
export const SETTINGS: SettingDef[] = [
  ...
  { name: 'NS_DEVICE_SUFFIXES', group: 'onebill', kind: 'config',
    gatedBy: 'ONEBILL_CLIENT_SECRET', example: '{"wp":{"label":"SNAPmobile Web"},"m":{"label":"SNAPmobile"},"t":{"label":"SNAPmobile Tablet"},"tm":{"label":"Teams","teams":true},"ai":{"label":"Acme App iOS"},"aa":{"label":"Acme App Android"},"aw":{"label":"Acme App Windows"}}',
    ...
    whenUnset: 'The four suffixes NetSapiens itself ships: wp SNAPmobile Web, m SNAPmobile, t SNAPmobile Tablet, tm Teams (TeamMate). A deployment whose TeamMate connectors still register as <ext>t must set this and mark t teams:true itself. Ringotel adds its activation suffix on top whenever that integration is enabled.',
    affects: ['onebill.view'] },
  ...
  { name: 'PORTAL_MENUS', group: 'menus', kind: 'config',
    example: '{"apps": {"hide": {"app": {"ringotel": ["SNAPmobile Web"], "none": []}}, "add": [{"label": "Support", "url": "https://support.example.com"}], "rename": [{"from": "SNAPAnalytics", "to": "Analytics Dashboard"}]}}',
    what: 'JSON adding, hiding or renaming entries in the Apps, account and Management menus. Each hide, add or rename is either a plain array, which applies to everyone, or an object targeting it on one axis: `users` by `user@domain`, `domains` and `scopes` by exact NetSapiens name, and `app` by whether your app is active for the user\'s domain — that last one takes an app name (`ringotel`, `documo`), `none` for a domain running none of them, or `*` for any state, and nothing else (an unknown key is a startup error rather than a rule that silently never matches). Precedence runs most specific first — account, domain, scope, app, then `*` — so naming an account carves an exception out of a domain-wide rule, and a `*` inside an axis is a default that never beats a rule naming you. A `rename` relabels a stock entry in place — same destination, same position, same icon — and its `from` is the entry as the portal ships it, which is the name a hide, a capture and the builder all keep using. Build it on the Menus tab rather than by hand: it works from your portal\'s real entries, which is the part no example can supply — including renames, where the tab pins the original name to the row instead of letting you type one.',
